...
from keras.callbacks import __________, ___________

# Save the best model as best_digits_model.hdf5
modelCheckpoint = ____________('best_digits_model.hdf5', save_best_only=True)

# Define a callback to monitor val_acc
monitor_val_acc = __________(monitor='val_acc', 
                       ______=5)

# Train the model using the early stopping callback
history = model.fit(x_train, y_train, 
                    ____________=(x_test, y_test),
                    _____=1000, batch_size=128,
                    callbacks=[___________])
