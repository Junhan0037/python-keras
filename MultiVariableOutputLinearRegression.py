# 선형회귀 : 다중 Input, Output
# 해당 선수와 같은 나이, 성별을 가진 선수들 차트에 표시 및 성별, 나이, 페이스를 학습하여 '10K', '20K',  '30K',  'Official Time' 예측
from tkinter import *
...
def histogram():
    ...
    if(t_g):
        ...
        gender_color = 'r'
    gender_record = df_2017[df_2017['M/F'] == t_g] # 해당 순위 선수와 같은 성별만 추출
    gender_age_record = gender_record[gender_record.Age == t_y] # 해당 순위 선수와 같은 나이만 추출
    gender_age_record_list = gender_age_record.values.tolist()
    
    grad_ax.plot(gender_record.Age, gender_record.Pace, '.', color=gender_color, alpha=0.5) # 해당 순위 선수와 성별, 나이가 같은 선수들 표시
    grad_ax.plot(t_y, t_p, 'yd') # 해당 순위 선수 표시
...
def learing(): 
    ...
    for i in range(len(time[0])):
        dist_time = int(time[0][i]) # 예측 시간
        real_time = seconds_to_hhmmss(y_test[t_a][i]) + '(' + str(y_test[t_a][i]) + ')' # 실제 시간
        ml_time = seconds_to_hhmmss(dist_time) + '(' + str(dist_time) + ')' # 예측 시간
        variation = y_test[t_a][i] - dist_time # 실제시간과 예측시간의 차이
